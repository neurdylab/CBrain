...
class CBrainRandom(nn.Module):
    ...
    def get_predictions(self, fmri_feats):
        ...
        random_predicted_name_result = torch.tensor(np.random.choice(values, size=(fmri_feats.shape[0], 1))).to(device)
        random_vigilance_result = torch.tensor(np.zeros((fmri_feats.shape[0], 1, 2))).float().to(device)
        predictions = {
            "vigilance_head_logits": random_vigilance_result,
            "vigilance_head": random_predicted_name_result,
        }
        return predictions
...
def build_random_guess(args, dataset_config):
